...
import { addMonths, addYears, differenceInMonths, differenceInYears, nextWednesday, setISODay } from "date-fns";
...
export const MntoLock = ({ className, style }: MntoLockProps) => {
    ...
    const performLock = () => {
        if (isValid) {
            showConfirm(
                `Do you want to lock ${getValues("toLock")} MENTO for ${[getYears(), getMonths()]
                    .filter(Boolean)
                    .join(" and ")}?`,
                {
                    confirmText: "Lock",
    ...
    return (
        <div className={className} style={style}>
            ...
            <div className="flex flex-col lg:flex-row justify-between md:place-items-baseline gap-1 md:gap-5">
                ...
                <div className="flex-1">
                    <DatePicker
                        ...
                        addon={<div className="text-xs opacity-50">Between 1 and 4 years</div>}
            ...
            <div className="flex flex-col mt-x6 lg:flex-row justify-between md:place-items-baseline gap-1 md:gap-5">
                <div className="text-lg flex-1 whitespace-nowrap">You recieve veMENTO:</div>
                <div className="flex-1">
                    <strong>
                        {isValid ? Math.round(watch("toLock") * 100 * (watch("expirationMonths") / 12)) : "¯\\_(ツ)_/¯"}
                    </strong>
                </div>
            </div>

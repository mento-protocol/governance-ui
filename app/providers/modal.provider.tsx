...
const ModalWrapper = ({ children, index, close, question, cancel, confirm, options }: ModalWrapperProps) => {
    return (
        <div key={index} className={classNames(modalStyles.modal, modalStyles.opened)} style={{ zIndex: 1000 + index }}>
                ...
                <Card className={classNames(modalStyles.modal__content__inner, modalStyles[options?.modalType || ""])}>
                    ...
                    {question && (
                        <Card.Footer className="flex justify-end gap-x2 mt-x4">
                            ...
                            <Button theme={modalTypeToButtonThemeMap(options?.modalType)} onClick={confirm}>
